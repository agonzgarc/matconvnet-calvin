classdef ImdbPartDetectionJointObjPrt < ImdbMatbox
    properties(SetAccess = protected, GetAccess = public)
        negOverlapRange = [0.1 0.3];
        posOverlap = 0.6;
        boxesPerIm = 128;
        boxRegress = true;
    ...
    methods
        function obj = ImdbPartDetectionJointObjPrt(imageDir, imExt, matboxDir, filenames, datasetIdx, meanIm)
        ...
        function [batchData, numElements] = getBatch(obj, batchInds, net, ~)
            ...
            if ismember(obj.datasetMode, {'train', 'val'})
                [boxesPrt, labelsPrt, overlapScoresPrt, regressionFactorsPrt] = obj.SamplePosAndNegFromGstructPrt(gStruct, obj.boxesPerIm);
                ...
                boxesPrt = obj.SampleAllBoxesFromGstructPrt(gStruct);
                boxesObj = obj.SampleAllBoxesFromGstructObj(gStruct);
        ...
        function [boxes, labels, overlapScores, regressionTargets] = SamplePosAndNegFromGstructPrt(obj, gStruct, numSamples)
        ...
        function [boxes, labels, overlapScores, regressionTargets] = SamplePosAndNegFromGstructObj(obj, gStruct, numSamples)
        ...
        function boxes = SampleAllBoxesFromGstructPrt(~,gStruct)
            boxes = gStruct.boxes(gStruct.boxesPrt,:);
        end

        function boxes = SampleAllBoxesFromGstructObj(~,gStruct)
